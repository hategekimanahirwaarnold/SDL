    ...
    } else
    {
        ...
        } else
        {
            ...
            while(!quit)
            {
				...
				//Calculate time step
				float timeStep = stepTimer.getTicks(&stepTimer) / 1000.f;

				//Move for time step
				dot->move(dot, timeStep );
				...
				//Render dot
	            instance.gDotTexture->render(instance.gDotTexture, &instance, (int)dot->mPosX, (int)dot->mPosY, NULL, 0, NULL, flipType);
