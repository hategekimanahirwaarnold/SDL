...
bool loadMedia(CL_Instance *instance)
{
    ...
    if (!instance->gBGTexture->loadFromFile(instance->gBGTexture, instance, "img/bg.png"))
    {
...
void move(Dot_s *self)
{
    ...
    if ((self->mPosX < 0) || (self->mPosX + self->DOT_WIDTH > LEVEL_WIDTH))
    {
        // Move back
        self->mPosX -= self->mVelX;
    }
    ...
    if ((self->mPosY < 0) || (self->mPosY + self->DOT_HEIGHT > LEVEL_HEIGHT))
    {
        // Move back
        self->mPosY -= self->mVelY;
    }
    ...
    else
    {
        ...
        else
        {
            ...
            while(!quit)
            {
                ...
                // Render background
                instance.gBGTexture->render(instance.gBGTexture, &instance, 0, 0, &camera, 0, NULL, flipType);

                //Show the dot relative to the camera
	            instance.gDotTexture->render(instance.gDotTexture, &instance, dot.mPosX - camera.x, dot.mPosY - camera.y, NULL, 0, NULL, flipType);
                // Update screen
