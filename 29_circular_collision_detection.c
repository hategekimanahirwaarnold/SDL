...
void move(Dot_s* self, SDL_Rect* square, Circle_s* circle)
{
    ...
    if( ( self->mPosX - self->mCollider->r < 0 ) || ( self->mPosX + self->mCollider->r > SCREEN_WIDTH ) || rectangle_checkCollision( self->mCollider, square ) || circle_checkCollision( self->mCollider, circle ) )
    {
        //Move back
        self->mPosX -= self->mVelX;
        self->shiftColliders(self);
    ...
    if( ( self->mPosY - self->mCollider->r < 0 ) || ( self->mPosY + self->mCollider->r > SCREEN_HEIGHT ) || rectangle_checkCollision( self->mCollider, square ) || circle_checkCollision( self->mCollider, circle ) )
    {
        //Move back
        self->mPosY -= self->mVelY;
        self->shiftColliders(self);
           ...
           while(!quit)
            {
                ...
                dot->move( dot, &wall, otherDot->getCollider(otherDot));
                ...
                // render texture
                instance.gDotTexture->render(instance.gDotTexture, &instance, dot->mPosX - dot->mCollider->r, dot->mPosY - dot->mCollider->r, NULL, 0, NULL, flipType);
                instance.gDotTexture->render(instance.gDotTexture, &instance, otherDot->mPosX - otherDot->mCollider->r, otherDot->mPosY - otherDot->mCollider->r, NULL, 0, NULL, flipType);
