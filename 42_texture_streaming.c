...
bool createBlank(lTexture_s* self, CL_Instance* instance, int width, int height )
{
	//Create uninitialized texture
	self->mTexture = SDL_CreateTexture( instance->gRenderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, width, height );
	if( self->mTexture == NULL )
	{
...
bool loadMedia(CL_Instance* instance)
{
	...
	//Load blank texture
	if( !instance->gStreamingTexture->createBlank(instance->gStreamingTexture, instance, 64, 205 ) )
	{
		printf( "Failed to create streaming texture!\n" );
		success = false;
	}
    if (instance->gDataStream->loadMedia)
	//Load data stream
	if( !instance->gDataStream->loadMedia(instance->gDataStream) )
	{		
...
int main()
{
    ...
    DataStream data;
    datastream_constructor(&data);
    texture_constructor(&instance.gStreamingTexture);
    instance.gDataStream = &data;
    ...
    else
    {
        ...
        else
        {
            ...
            while (!quit)
            {
				...
				instance.gStreamingTexture->lockTexture(instance.gStreamingTexture);
				instance.gStreamingTexture->copyRawPixels32(instance.gStreamingTexture, instance.gDataStream->getBuffer(instance.gDataStream) );
				instance.gStreamingTexture->unlockTexture(instance.gStreamingTexture);

				//Render frame
				instance.gStreamingTexture->render(instance.gStreamingTexture, &instance, ( SCREEN_WIDTH - instance.gStreamingTexture->mWidth ) / 2, ( SCREEN_HEIGHT - instance.gStreamingTexture->mHeight ) / 2, NULL, 0, NULL, SDL_FLIP_NONE );
